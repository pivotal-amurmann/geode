...
public class GetRequestOperationHandler
  ...
  public Result<RegionAPI.GetResponse> process(SerializationService serializationService,
    ...
    if (region == null) {
      return ProtobufResponseUtilities.createFailureResult("Region not found");
    }
    ...
    try {
      ...
      if (resultValue == null) {
        return ProtobufResponseUtilities.createNullGetResult();
      }

      BasicTypes.EncodedValue encodedValue =
          ProtobufUtilities.createEncodedValue(serializationService, resultValue);
      return ProtobufResponseUtilities.createGetResult(encodedValue);
    } catch (UnsupportedEncodingTypeException ex) {
      return ProtobufResponseUtilities.createFailureResult("Encoding not supported.");
    } catch (CodecNotRegisteredForTypeException ex) {
      return ProtobufResponseUtilities
          .createFailureResult("Codec error in protobuf deserialization.");
    }
  }
}
