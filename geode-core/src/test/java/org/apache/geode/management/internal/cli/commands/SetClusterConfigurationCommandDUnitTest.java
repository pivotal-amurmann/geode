...
public class SetClusterConfigurationCommandDUnitTest {
  ...
  public void noServerToBeginWith() throws IOException {
    ...
    gfsh.executeAndAssertThat(commandWithFile).statusIsError()
        .containsOutput("Can not set the cluster configuration if there are running servers");

    // you can not bounce the servers because they are not vanilla servers
    gfsh.executeAndAssertThat(commandWithFile + " --configure-running-servers").statusIsError()
        .containsOutput("Can not configure servers that are already configured");

    // you can only ignore with existing servers
    gfsh.executeAndAssertThat(commandWithFile + " --ignore-running-servers").statusIsSuccess()
  ...
  public void configureVanillaServers() throws IOException {
    ...
    gfsh.executeAndAssertThat(commandWithFile + " --group=groupA --configure-running-servers")
        .statusIsSuccess()
        .containsOutput("Successfully set the 'groupA' configuration to the content of")
        .containsOutput("Configure the servers in 'groupA' group")
        .containsOutput("Successfully configured server-1");
    ...
    gfsh.executeAndAssertThat(commandWithFile + " --group=groupB --configure-running-servers")
